...
bridge_interfaces=$(jq '.bridge_interfaces' $jsonFile)
bridge_interfaces=$(echo "$bridge_interfaces" | sed "s/[\"']//g")
...
echo "set bridge_interfaces=$bridge_interfaces" >> $tmpFile
